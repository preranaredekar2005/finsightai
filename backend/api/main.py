...
def compute_features(df: pd.DataFrame,
    ...
    row = {
        ...
        "ma5_cross":    ((close - close.rolling(5).mean()) /
                         (close.rolling(5).mean() + 1e-9)).iloc[-1],
        "ma10_cross":   ((close - close.rolling(10).mean()) /
                         (close.rolling(10).mean() + 1e-9)).iloc[-1],
        "ma20_cross":   ((close - close.rolling(20).mean()) /
                         (close.rolling(20).mean() + 1e-9)).iloc[-1],
        "ma50_cross":   ((close - close.rolling(50).mean()) /
                         (close.rolling(50).mean() + 1e-9)).iloc[-1],
        "vol_ratio":    vol_ratio.iloc[-1],
        "vol_change":   vol_chg.iloc[-1],
        "day_range":    ((df["High"] - df["Low"]) /
                         (close + 1e-9)).iloc[-1],
        "is_bullish":   int(close.iloc[-1] > df["Open"].iloc[-1]),
        "body_size":    (abs(close - df["Open"]) /
                         (close + 1e-9)).iloc[-1],
        "ret_l1":       ret.iloc[-2],
        "ret_l2":       ret.iloc[-3],
        "ret_l3":       ret.iloc[-4],
...
def get_price(ticker: str, period: str = "6mo"):
    try:
        ...
        return {
            ...
            "change":       round(float(data["Close"].iloc[-1] -
                                        data["Close"].iloc[-2]), 2),
            "change_pct":   round(float((data["Close"].iloc[-1] -
                                         data["Close"].iloc[-2]) /
                                         data["Close"].iloc[-2] * 100), 2),
...
def get_signal(ticker: str):
    try:
        ...
        # Get avg sentiment from DB
        sent_q = f"""
            SELECT AVG(sentiment_score) as avg_sent
            FROM news_articles WHERE ticker='{ticker}'
        """
